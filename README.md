...
* __waitForProcessExitTimeout__: Optional timeout in seconds used only when `waitForProcessExit=true`. `0` means wait without timeout. If the timeout is reached, the build fails. Default is 0.
